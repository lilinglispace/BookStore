    ...
    <div class="user_right">
       <div class="youbu">
           <div class="shi-head">
               ...
               <form action="<?php echo U('home/address/address');?>" method="post"  id="addressFrom">
                  <div class="guan-adr">
                      <ul>
                        <li class="guan-zuo"><i>*</i><span>所在地区:</span></li>
                        <li class="guan-you"> <input type="text" name="area" class="form-control" required id="city" />
                        <style type="text/css">
                    ...
                    <li class="guan-zuo1"><i>*</i>详细地址:</li>
                        <li class="guan-you1"><textarea class="wenziyu" placeholder="建议您如实填写详细收货地址，例如街道名称，门牌号码，楼层和房间号等信息"  required value="" name="desc_area" id="xiang"></textarea></li>
                    </ul>
                    </div>
                    <div class="guan-adr">
                    <ul>
                      <li class="guan-zuo"><i>*</i>邮政编码:</li>
                        <li class="guan-you">
                            <input type="text" class="bianma" name="post_code" required value="" placeholder="如您不清楚邮递区号，请填写000000" id="youbian">
                        </li>
                        </ul>
                    </div>
                     <div class="guan-adr">
                     <ul>
                      <li class="guan-zuo"><i>*</i>收货人姓名:</li>
                        <li class="guan-you">
                            <input type="text" class="bianma" placeholder="长度不超过25个字符" required name="name" value="" id="sname">
                            <input  type="hidden" name="userid" value="<?php echo (cookie('userid')); ?>" />
                        </li>
                     </ul>
                     </div>
                      ...
                      <ul>
                       <li class="guan-zuo"><i>*</i>收货人手机/电话:</li>
                        <li class="guan-you">
                            <input type="text" class="bianma"placeholder="长度不超过25个字符" required name="tel" value="" id="scall">
                        </li>
                      </ul>
                      </div>
                          <div class="addr"><input type="submit" value="添加" class="add-baocun"/></div>
                          <input type="hidden" name="id" value="<?php echo ($addr["id"]); ?>">
               </form>

             </div>
             <div class="yicun-address">
      <table border="0" cellspacing="0" cellpadding="0" class="tab-main">
       <tbody>
         <tr class="th-tou">
           <th>收货人</th>
           <th>所在地区</th>
         ...
         <?php if(is_array($add)): $i = 0; $__LIST__ = $add;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="tab-con address-default">
          <td><?php echo ($vo["name"]); ?></td>
          <td><?php echo ($vo["area"]); ?></td>
          <td><?php echo ($vo["desc_area"]); ?></td>
          <td><?php echo ($vo["post_code"]); ?></td>
          <td><?php echo ($vo["tel"]); ?></td>
          <td>
          <a href="<?php echo U('home/address/edit',array('id'=>$vo['id']));?>">修改</a>|
          <a href="<?php echo U('home/address/del',array('id'=>$vo['id']));?>" onclick="return del()">删除</a>|
          <?php if(($vo["status"] != 1)): ?><a href="<?php echo U('home/address/mo',array('id'=>$vo['id']));?>">设为默认地址</a>
              <?php else: ?>默认地址<?php endif; ?>
